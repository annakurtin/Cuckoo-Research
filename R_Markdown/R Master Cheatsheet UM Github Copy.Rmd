...
Thing to put in YAML to adjust output:
header-includes:
  - \usepackage{lineno}
  - \linenumbers
  - \usepackage{setspace}
...
# Just good to know
## Collapse all code chunks with
## Expand all code chunks with alt+shift+O

```

[Making manuscripts in R](https://jhemberger.github.io/posts/posts/r-markdown-manuscripts/)
...
# For working with LiDAR data: lidR
...
# For conducting bayesian analysis on breeding bird survey data: bbsBayes https://www.rdocumentation.org/packages/bbsBayes/versions/2.5.3
...
# github resources for when you try to commit something that's too big and need to uncommit it
# https://stackoverflow.com/questions/40115723/undo-git-commit-in-rstudio-that-is-too-big-to-push
# in command line, navigate to directory of project
# once inside tye git reset HEAD~
# repeat as many times as the commits you need to undo

# for storing large files through github, use lfs: https://git-lfs.com/


# Can suppress warnings using suppressWarnings()
suppressWarnings({
  print("whoops!")
})
```
...
# ifelse doesn't work well with dates, if you want to use this expression dates use if_else() the dplyr version
if(test_expression){
  #Statement
} else {
  # Statement
...
# To print things without spaces as the separator
paste0(string1,"end")
# you can also specify the separator
paste(string1,"end",sep = "_")
print(combo_string)
...
# ex: if you want to take the max value across other columns and create a new column with it:
#pb_dat$bbcu <- apply(pb_dat[, 2:4], 1, max, na.rm = TRUE)
...
# You can also import or export R data objects, which will save the formatting in columns such as datetime and prevent you having to go between .csv files

# There are two ways to do this
# You can save it in R's native binary with save() and load()
save(samp_occ_l_summ, file = "./Data/Detection_History/2023_All_ARUs/Outputs/fit_varying_samplength_occ.Rdata")
occ_output <- load(samp_occ_l_summ, file = "./Data/Detection_History/2023_All_ARUs/Outputs/fit_varying_samplength_occ.Rdata")

# OR as an Rdata file with saveRDS and readRDS
#For saving models (sum_det_mods is a named list object):
saveRDS(sum_det_mods,file = "3.Outputs/Red_Fox_Models/summer_coyote_state_physical_models.RData")
#for reading them back in:
sum_det_mods <- readRDS("3.Outputs/Red_Fox_Models/summer_coyote_state_physical_models.RData")
...
i = where/order by (row)
j = select/update (column)
by = group by 

- "Take DT, subset/reorder rows using i, then calculate j, grouped by by"
- Run things more like python - run the whole script at once 
- Anything that takes a data frame will also take a data table 

Waldo's recommendation: have a separate data cleaning script where you have an output and bring it into another script 
...
# Reading in data to a data table
photos <- fread("practice_data\\sample_photo_data.csv")
# if it's not recognizing the column names
photos <- fread("practice_data\\sample_photo_data.csv", header=TRUE)

# Options when reading in data tables
# read in only a couple of columns
fread("data", select = c("stationID","photo_dtm"))

# drop a couple of columns
fread("data",drop = c())
# Can read in a certain number of rows
fread("data", nrows = 20)

# Data.tables work with keys
## defaults to row numbers but you can specify what you want to use as the key
...
# pull out rows with spp = MACA
photos[species == "MACA"]

# subset by more than one variable name
photos[species== "MACA" & Number_of_Animals > 1]

# pull out a whole column as a vector
photos[ , stationID]
# leave the comma to say you want to skip i (include all rows)


#pull out the whole column as a data.table
photos[ , .(stationID)]
# multiple columns
photos[ , .(stationID, species)]
## can also use c()
# can rename when you pull them out
photos[ , .(stationID, sp_code = species)]

# if you're changing something about the data.table it will automatically update
## if you're pulling something out you have to reassign
## to test your data, make a copy of it 
# always copy it first then play around withit
photos2 <- copy(photos)
class(photos2)
photos3 <- photos
class(photos3)
# make sure that you use copy() because assigning them as the same thing will link them together and one change in it will also change what its associated with 

# can store col names and refer to them with .. (one level up = global enviro)
select_cols <- c("StationID","species")
photos[ , ..select_cols]
 
# can pull out columns between columns

# can use ! to specify not this and - to specify the reverse order

# sort by columns with the order function  
photos[order(stationID,photo_dtm,-species)]

# Rename one column
colnames(UMBEL_aru_dt)[7] <- "2022_status"

# can use set attribute functions to modify dataset
setorder() 
setnames()
setcolorder()
# will move columns not specified to the end 
# all functions and prior automatically reassign the value, but you can reassign it and use the function copy() to create a new value

# to update/add a column for all rows
photos[, project := "Camera Project"]

# have to refresh the environment to see the changes you made take place 

# can specify more than one columns adding at a time - functional form
photos[, ":=" (project = "Sample Data"),
       photo_num = substr(photo_id, 1, 6),
       location = "Montana"]

# removing columns
photos[ , c("location") := NULL]
# will completely wipe it from the memory, different from dropping it with a minus

# can also compute within columns
photos[, sum(species== "TADO")]
photos[, sum(ind_count > 1, na.rm = TRUE)]

# update/add/calculate a column for only select rows
photos[species == "TADO", common_name := "Douglas Squirrel"]
# FILL IN ___________________

# can also chain/pipe things together
photos[species == "MACA", common_name := "Douglas Squirrel"
       ][species == "MAAM", common_name := "Marten"]

# .N and how its used
# holds number of observation in a group or number of rows in a subset
photos[species == "ODHE", .N]

# by - works as group by 
photos[, .(.N), by = species]

# can also still subset by other columns 
photos[month(photo_dtm) == 7, .N, by = species]

# can do calculations with grouping
photos[month(photo_dtm)==7, mean(photo_dtm), by = species]
# takes each month of number 7 and takes the mean of the date time for each species 
## month works with datetime values 

# by vs keyby (keyby is more when you assign keys)
## use this with a SQL database 

# can use calculations within grouping
# groups by first part but then by second part if it is true or false
photos[, .N, by = .(species, month(photo_dtm) > 7)]

# .SD denoates a subset of data
photos[, print(.SD), by = species]
# will make a whole new table for each species and save all in a list 

# can combine with lapply
photos[year(photo_dtm) == 2019,
       lapply(.SD, mean, na.rm = TRUE),
       by = species,
       .SDcols = c("photo_dtm", "station", "ind_count")]
# takes the mean for each species from 2019

# melt and decast for pivoting long and pivoting wider 
# should be able to use pivot longer and pivot wider in data.table
## can use join and merge
## all tidyverse functions with data.table

# joining
photos[photos2, on = "column to merge on"]
...
# reference code for making columns long and then combining them
...
# add on merge if you want to bypass warnings about finding more or less than the items specified
# add on remove = FALSE if you want to keep the orignal column
r7PB_22 <- r7PB_22 %>% separate(point_id, into = c("site_id", "point"), sep = "-" , remove = FALSE, extra = "merge")
...
dataframe %>% 
  unite(col = newcolumn, c("old_col1","old_col2"), sep="")
...
# look for this in the help file
# to convert an integer to a date
r6_diel2$date <- as.Date(as.character(r6_diel2$date),format="%Y%m%d")


wolf_summer5 <- wolf_summer4 %>% mutate(datetime_new=as.POSIXct(datetime,format="%m/%d/%y %H:%M"))
# format that you specify for what it's coming in as

# You can also create a column that's just time values
pbmeta <- pbmeta %>%
...
pbmeta <- pbmeta %>%
  mutate(time = times(time_only, format = "h:m:s"))
...
# How to replace blanks with NA values
...
# Change a list within a dataframe to a column
neg_dat <- neg_dat %>%
  mutate(
    sample_start = as.numeric(unlist(sample_start)),
    sample_end = as.numeric(unlist(sample_end))
  )
...
# To change a character to a factor and specify the levels
...
[Useful resource on removing rows, including removing duplicates](https://favtutor.com/blogs/remove-row-in-r)
...
# Pulling out certain rows in R
## To pull out data in a way that forms another dataframe, index it in brackets
...
# can also do this with character strings
rh_dat <- playback_dat %>% filter(grepl("84|83|203|82", site_id))

# Can filter out rows containing an unwanted value
...
# can also use ! to filter things not in a value
count_arus <- function(data){
  ARUs_touse <- data %>% filter(use_2023 == "yes") 
  ARUS_touse <- ARUs_touse %>% filter(!use_as_b_team %in% "yes") # here is the example
...
# Can use base R to drop one row that has a specific value in one column
site_table <- site_table[!(site_table$species_code %in% "NOBI"),]
...
# to get/sample random rows from the dataframe: sample_n
...
## Using case_when with regular expressions:
test <- veg %>%
  mutate(sampling_design = case_when(
    grepl("^[[:alpha:]]{4}-[[:digit:]]{3}$", point_id) ~ "habitat_grts", # match four alphanumeric then a dash then three digits
    grepl("^[[:digit:]]{2,3}-[[:digit:]]{1}$", point_id) ~ "mmr_grts", # match two or three digits then a dash then one digit
    grepl("^[[:alpha:]]{3}-[[:digit:]]{1}$", point_id) ~ "selectedcu_nonrand", # match three alphanumeric then a dash then one digit
...
# Testing if values are approximately equal to each other (diffrent than == or !=) using all.equal()
merged_df <- aru_23 %>%
...
tree_counts <- tree %>% count(tree_species, sampling_design) %>% arrange(desc(n))

# To just pull out the first instance of a character string:
ex <- tree %>% group_by(site_id) %>% reframe(sampling_design = first(sampling_design))
...
To update join: https://stackoverflow.com/questions/51572462/update-replace-values-in-dataframe-with-tidyverse-join 
* Update join is specifically included in data table
...
`full_join(df1,df2, by="ID")` full_join(x, y) keeps all of the rows from both datasets and just inserts NAs where there is missing information
...
# To string together multiple left_joins:
dat_long <- dates_long %>% left_join(effort_long, by = "site_survey") %>% left_join(backdb_long, by = "site_survey")
...
# You can also join two columns and remove the NAs in them with coalesce()
...
# other option
test <- umbel_playb_points %>% rows_update(umbel_tocombine,by="point_id") # Note: all of the rows in umbel_tocombine must exist in the larger data set you're trying to put them into (umbel_playb_points)
...
# Examples: picking out values in two dataframes that are common between the two or are unique to one
...
```{r}
# Resampling from another column of data
# create a data frame with a column to resample
df1 <- data.frame(col1 = c(1, 2, 3, 4, 5))

# create a data frame to add data to
df2 <- data.frame(col2 = c("A", "B", "C"))


# resample col1 from df1 and add it to df2
df2$new_col <- sample(df1$col1, size = nrow(df2), replace = FALSE)
...
# why does this put a [,1] if you don't have the c()? becuase scale() function returns a matrix
...
# writing a CSV file (write_csv or write.csv)
#write.csv(DataFrame,"Path\\FileName.csv",append = FALSE, row.names = FALSE)
...
## You can add row.names=FALSE if you don't want the first column of row names written
...
[Functions to pull out information about rasters](https://rdrr.io/github/rspatial/terra/man/dimensions.html)

[R and GIS for Economists](https://tmieno2.github.io/R-as-GIS-for-Economists/raster-data-object-classes.html)

[For making a hexagonal tessellation](https://search.r-project.org/CRAN/refmans/spatstat.geom/html/hextess.html)

[To Find Which Projection You Want](https://epsg.io/32100)

### Notes and definitions

Easting is the x coordinate and northing is the y coordinate in UTM projections

```{r Working with Vector Data}
#Step 1: load in the data
## Loading in shapefiles
shapefile <- st_read("Path\\To\\File")
...
## Method 2: mapview
## good for quickly visualizing things or visualizing things in an interactive way
## Resource: https://r-spatial.github.io/mapview/
mapview(locs)
...
## Method 4: ggmap
### only works with ggmap objects, which is what stamenmap automatical=lly gives you
basemap_proj <- get_stamenmap(as.numeric(proj_bound),maptype = "terrain-background", zoom=6)
ggmap(basemap_proj)
# but you can also plot other objects on top of it
ggmap(basemap_proj)+geom_sf(data = locs_sf,
                            inherit.aes=FALSE,
...
# if the plots start visualizing weird in the corner, run this code to clear plot settings
dev.off()
...
# Other operations: setting a buffer around a polygon or line
...
For most plotting, you'll also need a basemap. There are several different options for this:

[CalTopo](https://caltopo.com/map.html#ll=33.20829,-109.73145&z=8&b=mbt)

[Stamen Maps](http://maps.stamen.com/#toner/12/37.7706/-122.3782)

[More info on stamen maps](https://www.r-bloggers.com/2018/10/getting-started-stamen-maps-with-ggmap/)

```{r Base Maps}
# Importing from CalTopo

# Importing from Stamen Maps
#1. set bounding box
MT_bound <- st_bbox(locs_dat)
#xmin, ymin, xmax, ymax

# Changing bbox boundaries
# lets reset the boundaries to what we want, the order is:
#xmin, ymin, xmax, ymax
MT_bound[1] <- xmin_proj
MT_bound[2] <- ymin_proj
MT_bound[3] <- xmax_proj
MT_bound[4] <- ymax_proj

# You can also edit it based on the existing data
MT_bound[1] <- MT_bound[1]-5# assign it a number 
# might require you to put names in 
## can go through and add buffer

# can transform a bbox into an sf object
...
Can be a different beast in R.

[Reading in .tiff files with their auxillary data in R](https://gis.stackexchange.com/questions/273372/how-to-access-the-attribute-table-of-a-tif-map-in-r)

[Making Dummy Raster Data in R](https://rspatial.org/spatial/8-rastermanip.html)

### A Note On Lining up Raster Data

Here we will go through some typical raster operations when creating a set of spatially consistent raster layers. Our goal is to create a consistent stack of rasters - a raster stack, or brick, of the same raster extent, resolution and projection for all subsequent analyses in R. 
...
```{r Working with Raster Data}
# Quick and dirty make a dummy raster
dummy <- terra::rast(ncol=36,nrow=18,xmin=-109.880833,xmax=-108.288889,ymin=47.551111,ymax=47.825278)
values(dummy) <- 1
crs(dummy) <- "+proj=lcc +lat_0=44.25 +lon_0=-109.5 +lat_1=49 +lat_2=45 +x_0=600000 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs"
...
# If you import a .tif file from an arcGIS file that has more .tif.aux files you need both the .tif and the .aux file
...
plot(elev)
plot(lcov, add=T)
# A little bit about plotting rasters using plot() and taking rasters from GIS files https://gis.stackexchange.com/questions/276694/visualizing-raster-in-r
...
# cropped_data <- crop(data_orig, data_to_match, snap="near")
...
# Another way to do this: masking (listed above)
#1.  Create a mask raster to use as a template for converting shapefile data to rasters
...
# next resample the raster data to make it match - continuous data
...
## method = "bilinear" means bilinear interpolation and uses a weighted average of the four nearest cell centers. Best used for continuous data.
## method = "near" mean nearest neighbor and the value of the output cell is determined by the nearest cell center on the input grid. Best used for categorical data. 
...
# You may also need to reproject your raster
...
```{R Plotting Spatial Data Togehter }
...
#tmap resources: https://rpubs.com/quarcs-lab/tutorial-maps-in-r
# more tmap resources: https://cran.r-project.org/web/packages/tmap/vignettes/tmap-getstarted.html#multiple-shapes-and-layers
...
[Combining Vector and Raster Data](https://doi90.github.io/EE_Intro_Spatial_Workshop/combining-vector-and-raster-data.html)

[Data Carpentry: Manipulating Raster Data](https://datacarpentry.org/r-raster-vector-geospatial/11-vector-raster-integration/)

[Spatial Data Science: Raster data manipulation](https://rspatial.org/spatial-terra/8-rastermanip.html)

[NSF NeonScience: Raster Data in R](https://www.neonscience.org/resources/learning-hub/tutorials/raster-data-r)

[Getting National Land Cover Data](https://www.rdocumentation.org/packages/FedData/versions/2.5.7/topics/get_nlcd)
...
# how to pull one layer out of a raster stack 
elevation <- landcover_stack[[3]]
```
...
[Data Carpentry Resource](https://datacarpentry.org/semester-biology/materials/spatial-data-saving-R/)

```{R}
# Export rasters with terra::writeRaster(https://tmieno2.github.io/R-as-GIS-for-Economists/read-and-write-a-raster-data-file.html)
# terra::writeRaster(spatial_object,"Path\\FileName.tiff")
# for saving polygons
# write_sf(object)
...
```{r}
# working with NLCD Data from Lab 10 Habitat Modeling
```

# Sampling Design

When designing a survey, you will need to choose locations to sample. The way you choose these samples has important implications for your statistical inference. For a more detailed explanation, refer back to Research Design notes from Fall 2022 with Paul Lukas. How to implement these sampling protocols in R is outlined below.

**Resources:**
[Stratified Random Sampling in R](https://www.r-bloggers.com/2022/02/stratified-sampling-in-r-with-examples/)

[Stratified Random Sampling with Spatial Data](https://rdrr.io/cran/spatialEco/man/stratified.random.html)

[Stratified Sampling in R With Examples](https://finnstats.com/index.php/2022/02/20/stratified-sampling-in-r/)

[Ch 4: Stratified simple random sampling](https://dickbrus.github.io/SpatialSamplingwithR/STSI.html)

```{R Sampling}
# Fill this in with code
# spatialEco::stratified.random(x, strata, n = 10, reps = 1, replace = FALSE)
# sampling::strata

# hexagonal grid/tesselation: https://search.r-project.org/CRAN/refmans/spatstat.geom/html/hextess.html
...
[Good explanation of the basics](https://www.youtube.com/watch?v=FgakZw6K1QQ)
...
## PCA Ex 2
[From StatQuest Video](https://www.youtube.com/watch?v=0Jp4gsfOLMs)
[Code from GitHub](https://github.com/StatQuest/pca_demo/blob/master/pca_demo.R)


```{r PCA Example 2}
# Create a dataset
data_matrix <- matrix(nrow = 100, ncol = 10) # create a dataset of 100 genes of 10 individuals
colnames(data_matrix) <- c(
  paste("wt", 1:5, sep = ""), # the first 5 samples are wild type
  paste("ko", 1:5, sep = "")) # the last 5 are kockout

rownames(data_matrix) <- paste("gene", 1:100, sep="") # name the genes
for (i in 1:100) {
  wt.values <- rpois(5, lambda=sample(x=10:1000, size=1))
  ko.values <- rpois(5, lambda=sample(x=10:1000, size=1))
 
  data_matrix[i,] <- c(wt.values, ko.values)
} # give the fake genes fake read counts using the poisson distribution

# take a look at the first couple rows
head(data_matrix)
dim(data.matrix)

# Now call the function prcomp() to do a PCA on our data
## Goal is to draw a graph to show how variables are related to one another
pca <- prcomp(t(data_matrix), scale=TRUE) # prcomp() expects samples to be rows and genes to be columns, so we transpose it using t() here

# prcomp returns three things: 
#1. x - contains the principal components (PCs) for drawing a graph
## plot pc1 and pc2 using the first two columns in x (first two PCs)
plot(pca$x[,1], pca$x[,2])
## Notice here that 5 samples are on one side, five on the other side. Let's see how much variation in the original data the first PC accounts for

#2. sdev - standard deviation, which we will take the square of here to see how much variation in the original data each principal component accounts for
pca_var <- pca$sdev^2
## also calculate the percentage that it accounts for
pca_var_per <- round(pca_var/sum(pca_var)*100, 1)
## Which we can use to make a scree plot
barplot(pca_var_per, main="Scree Plot", xlab="Principal Component", ylab="Percent Variation")
## Here we can see that PC1 accounts for nearly all of the variation in the data, meaning that there is a big difference between the clusters to the left and right of our original graph


# Make a fancy GG plot
...
## lets pull it up to see which genes have the largest effect on the plot
## Look at the loading score for PC1, since it accounts for the most amount of variation
loading_scores <- pca$rotation[,1]
## use the absolute value function to only look at magnitude of loading score and sort them
gene_scores <- abs(loading_scores) 
gene_score_ranked <- sort(gene_scores, decreasing=TRUE) # sort them
top_10_genes <- names(gene_score_ranked[1:10]) # pull out the name of the genes
...
[Combining ggplots with ggpubr](http://www.sthda.com/english/articles/32-r-graphics-essentials/126-combine-multiple-ggplots-in-one-graph/)
...
# To change the order in which your categories/factors are displayed:
...
For showing density plots

[resource on geom_violin](http://www.sthda.com/english/wiki/ggplot2-violin-plot-quick-start-guide-r-software-and-data-visualization)

```{R}
ggplot(data = pam_pstars_long, aes(x = num_surveys, y = cumulative_p)) + 
  geom_violin(fill = cuckoo_palette[1])
```
## Facet Wrapping
...
# to change the background and line color of your plot:
theme_minimal() +
...
```{R}
# example code
df <- data.frame(parameter = sample(size = 1000,replace = TRUE, c("veg density", "background noise", "effort", "date")),
                 values = rnorm(n = 1000, mean = 0, sd = 1))
  
# Create the slab interval plot based on example code
ggplot(data = df, aes(x = values, y = parameter, fill = parameter)) + 
  stat_slabinterval() +
...
```{R Exporting Publication Quality Graphics}
...
# jpg/jpeg
jpeg("Output/wolf_elev.jpeg", width=4, height=4)
print(ggplot(wolfkde, aes(x=Elevation2, y=used)) + geom_point(colour="gray") + stat_smooth(method="glm", method.args = list(family="binomial")) + xlab("Elevation (m)") + ylab("Predicted Probability of Wolf Use"))
# turn the printing off 
dev.off()
```
...
Arguments to rbonim() [explanation here](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/Binomial)

1. n - the number of observations/ data points you want to create

2. size - the number of trails in a binomial distribution. For a Bernoulli, this is always 1. 

3. prob - the probability of success on each trial
...
# Generating data with rbinom 
#Ex: 85 total habitat points, 18 with detections, 21% naive occupancy 
bbcu_pres <- rbinom(n = 85,size = 1,prob = .21)
...
[Occupancy models in R with code examples](https://jamesepaterson.github.io/jamespatersonblog/2021-01-01_dynamicoccupancy.html)

[Package unmarked for occupancy analysis](https://cran.r-project.org/web/packages/unmarked/vignettes/unmarked.html)
...
# Making and Maintaining a Website in Github
