...
clips_23 <- read.csv("./Data/Classifier_Results/Model2.0/Outputs/2023_AllCollab_topclips_filteredPBYBCU_5-27.csv") %>% mutate(datetime = as_datetime(datetime,  tz = "America/Denver")) %>% mutate(date_formatted = as.Date(date_formatted, format = "%Y-%m-%d")) %>% create_site_col()


# Create visualizations
det_23 <- clips_23 %>% group_by(site_id, date_formatted) %>% summarize(detection = max(annotation))
...
ggplot(sites_positive, aes(x = date_formatted, y = detection)) +
