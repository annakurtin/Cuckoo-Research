...
---
This is the fourth iteration of the full occupacy model. From each of the submodels, I carried forward any covariate with f statistic > .70. I assessed covariance of the parameters in the previous model and assessed covariate stability, and discarded subcanopy average height (a5) from this model. This covariate had unstable signs of the parameter effects and was strongly colinear with the parameter estimates of percent subcanopy core (f = -0.483; meaning you can't increase or decrease the effect of one without decreasing or increasing the effect of another).

I went back through and added the effect of conifer species richness after rerunning this submodel. I realized that we want inference on not only deciduous species richness but also to be able to say if they're using sites with conifers at all. 

Still pending the results from why my baseline estimate for occupancy is low and why my waic is low.
...
* a8: conifer species richness on a point scale (from submodel A3)


$$
z_i \sim Bernoulli(\psi | \alpha_{1}, \alpha_{2}, \alpha_{3}, \alpha_{4}, \alpha_{6}, \alpha_{7}, \alpha_{8})
$$ 
...
corr_dat <- full_dat %>% select(dtree_spp_rich, pct_can_landsc, pct_subcan_landsc, pct_subcan_core, veg_sd_resid, floodplain_shrub, ctree_spp_rich)
...
hist(full_dat$dtree_spp_rich, main = "Decid tree Spp Richness")
hist(full_dat$ctree_spp_rich, main = "Conif tree Spp Richness")
...
```{R Univariate Relationships, include = FALSE}
...
jags_data <- list(
  ...
  # conifer tree spp richness
  cova8 = full_dat$ctree_spp_rich,
  # Date
...
cat("
  model{
    ...
    for(i in 1:n_unit){
      ...
      logit(psi[i]) <- a0 + a1*cova1[i] + a2*cova2[i] + a3*cova3[i] + a4*cova4[i] + a6*cova6[i] + a7*cova7[i]+ a8*cova8[i]
  ...
  ", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Full_Occupancy_Model/Model_Structure_Files/JAGS_GlobMod4.txt")

```

```{R Run Model, results = FALSE}
fit_g4 <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Full_Occupancy_Model/Model_Structure_Files/JAGS_GlobMod4.txt",
                 parameters.to.save = c("a0", "a1", "a2", "a3", "a4", "a6", "a7", "a8", "b0", "b1", "b1Q", "b3", "b4","p_val"),
                 n.iter = 10000, 
                 n.burnin = 4000, 
                 n.chains = 3,
                 n.thin = 2,
...
saveRDS(fit_g4,"C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Full_Occupancy_Model/Models_Ran_Outputs/HabChap_GlobMod4.Rdata")

```

```{R}
fit_g4 <- readRDS("C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Full_Occupancy_Model/Models_Ran_Outputs/HabChap_GlobMod4.Rdata")
```

```{R}
summary(fit_g4)
tracedens_jags(fit_g4, parmfrow = c(3,3))
```
...
* cova8: Native conifer tree spp richness



```{R Create Violin Plots, echo = FALSE}
chains_g4 <- jags_df(fit_g4)
# Select the chains for our covariates of interest
chains_viol <- chains_g4 %>% select(a0, a1,a2, a3, a4, a6, a7, a8)
# Rename them to be more interpretable
colnames(chains_viol) <- c("psi","decid tree spp rich", "% can landsc", "% subcan landsc", "% subcan core", "residual veg sd", "floodplain shrub","conif tree spp rich")
# Pivot this longer so that we can visualize it
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("psi", "decid tree spp rich",  "% can landsc", "% subcan landsc", "% subcan core", "residual veg sd", "floodplain shrub","conif tree spp rich"),names_to = "Parameter", values_to = "Values")
med_vals <- data.frame(vals = c(fit_g4$q50$a0,
                       fit_g4$q50$a1,
                       fit_g4$q50$a2,
                       fit_g4$q50$a3,
                       fit_g4$q50$a4,
...
ggplot() +
  ...
  geom_point(data = med_vals, aes(x = names, y = vals)) +
  labs(title = "Posterior Distribution of Global Model Covariates") +
  scale_fill_manual(values = c("psi" = palette_8[8], "decid tree spp rich"=palette_8[1], "% can landsc" = palette_8[2], "% subcan landsc" = palette_8[3], "% subcan core" = palette_8[4],  "residual veg sd" = palette_8[6], "floodplain shrub" = palette_8[7], "conif tree spp rich" = palette_8[8])) + 
  geom_hline(yintercept = 0, linetype = "solid", color = "black") +
  theme_minimal()

chains_beta <- chains_g4 %>% select(b1, b1Q, b3, b4)
...
```{R Format Data for Caterpillar Plot, echo = FALSE}
# Make the summary of the model into a dataframe
cat_dat <- as.data.frame(fit_g4$summary)
# Clean and change names, convert rownames into the columns
cat_dat <- cat_dat %>% clean_names() %>% rename(ci_lower = x2_5_percent,median = x50_percent,ci_upper = x97_5_percent) %>% rownames_to_column("param")
cat_dat <- cat_dat %>% mutate(f_stat = round(f,2))
# Select columns of interest
cat_dat <- cat_dat %>% select(param, ci_lower, median, ci_upper, overlap0, f_stat)
# Drop p val and deviance
cat_dat <- cat_dat[2:8,]
# Change names to reflect actual parameters of interest
cat_dat <- cat_dat %>% mutate(name = case_when(param == "a1" ~ "decid tree rich",
                             param == "a2" ~ "% can landsc",
                             param == "a3" ~ "% subcan landsc",
                             param == "a4" ~ "% subcan core",
                             param == "a6" ~ "resid veg sd",
                             param == "a7" ~ "floodplain shrub",
                             param == "a8" ~ "conifer tree rich"))
```
```{R Caterpillar Plots}
# Plot this
ggplot(data = cat_dat) +
  geom_segment(mapping = aes(x = ci_lower, xend = ci_upper, y = name), linewidth = 2)+
  geom_vline(xintercept = 0) +
  geom_text(aes(x = ci_upper, y = name, label = f_stat), hjust = -0.2, color = "blue") +
  labs(x = "95% Credible Interval", y = "Parameter") +
  theme_minimal()

```


```{R WAIC G3, echo = FALSE, eval = FALSE}
# Extract samples from the posterior
g4_s <- jags.samples(fit_g4$model,
                           c("WAIC","deviance"),
                           type = "mean",
                           n.iter = 5000,
                           n.burnin = 1000,
...
g4_s$p_waic <- g4_s$WAIC
# Calculate a new waic by adding the waic plus the deviance
g4_s$waic_calc <- g4_s$deviance + g4_s$p_waic
...
waic_g4 <- round(c(waic = tmp[["waic_calc"]], p_waic = tmp[["p_waic"]]),1)
# Calculate the standard error for WAIC
g4_waic_se <- sd(g4_s$waic_calc)/(sqrt(length(g4_s$waic_calc)))
```


```{R Look at Parameter Correlation}
# Create a matrix of the mcmc chains for each parameter
mat <- cbind(fit_g4$sims.list$a0,fit_g4$sims.list$a1,fit_g4$sims.list$a2,
             fit_g4$sims.list$a3,fit_g4$sims.list$a4,
             fit_g4$sims.list$a6,fit_g4$sims.list$a7, fit_g4$sims.list$a8)
...
* Why is our intercept estimate baised low? (Pending input from Thomas R)


```{R Graveyard, include = FALSE, eval = FALSE}
# Format data from the summary
cat_dat1 <- data.frame(names = c("psi",
                                 "decid tree spp rich", 
                                 "% can landsc", 
                                 "% subcan landsc", 
                                 "% subcan core", 
                                 ...
                                 "p_val",
                                 "deviance"),
                       dist_lower = list(fit_g4$q2.5))
cat_dat <- data.frame(names =  c("psi",
                                 "decid tree spp rich", 
                                 "% can landsc", 
                                 "% subcan landsc", 
                                 "% subcan core", 
