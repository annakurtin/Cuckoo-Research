...
This model is all of the parameters plus a quadratic effect on date. This is the same model structure as submodel 5 but using the updated cleaned data to make sure there's not a difference when using it.
    ...
    for(i in 1:n_unit){
      ...
      for(j in 1:miss[i]){
       # Impute missing detection data
        ...
        p[i,j] <- b0 + b1*cov1[i] + b2*cov2[i,j] + b3*cov3[i,j] + b4*cov4[i,j] + b4Q*(cov4[i,j]^2)
...
habdet_f2 <- jags(data = jags_data, 
                 ...
                 parameters.to.save = c("a0", "b0", "b1", "b2", "b3", "b4","b4Q", "p_val", "SSEobs","SSEsim"),
                 n.iter = 10000, 
                 n.burnin = 4000, 
                 n.chains = 3,
                 n.thin = 2,
...
```{R}
summary(habdet_f2)
tracedens_jags(habdet_f2, parmfrow = c(2,3))
```

### Examine Model Fit:

```{R Make Inference on P Value}
plot(habdet_f2$sims.list$SSEobs,habdet_f2$sims.list$SSEsim,xlab="SSEobs",ylab="SSEsim")
abline(0,1,lwd = 2, col = "red")
print(paste0("P value is:",round(habdet_f2$mean$p_val, 2)))
```
...
p <- round(plogis(habdet_f2$q50$b0),4)
...
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("veg density",
                                                          ...
                                                          "effort",
                                                          "date", 
                                                          "quadratic date"),names_to = "parameter", values_to = "values")
...
f_stat <- data.frame(
  parameter = c("veg density",
                ...
                "effort","date", 
                "quadratic date"),
  median_value = c(paste0("F: ", round(habdet_f2$f$b1,2)), 
                   paste0("F: ",round(habdet_f2$f$b2,2)), 
                   paste0("F: ",round(habdet_f2$f$b3,2)), 
                   paste0("F: ",round(habdet_f2$f$b4,2)), 
                   paste0("F: ",round(habdet_f2$f$b4Q,2)))
)
...
I'm extracting the WAIC for this model to compare it to versions of the detection model that don't include a quadratic effect of date (Detection Model Final 1) and that have a quadratic effect on other covariates (Detection Model Final 3).
...
* Date: *How to interpret this?*

* Quadratic effect of date: *How to interpret this?*
...
* Effort: There is a 73% chance that the true effect of survey effort at a site during a given survey period on probability of detecting a BBCU given that they are present at a site is positive. The median estimate for the effect of survey effort on detection probability is 0.533, which means that for every 1 sd increase in survey effort above the mean (10 increase above 17).
