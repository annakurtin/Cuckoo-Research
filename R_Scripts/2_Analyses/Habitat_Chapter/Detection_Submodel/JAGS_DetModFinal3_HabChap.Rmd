---
title: "Habitat Chapter Detection Sub Model Final 3"
author: "Anna Kurtin"
date: "8/26/2024"
...
This model is all of the parameters plus a quadratic effect on background db. Looking at the distribution of the covariates, it looks like there might be a quadratic effect, since most of the detections are at intermediate levels of background db. However, I have a hunch that this might be just because the data is sparse on either end of this covariate. Additionally, the quiet nights with background noise way lower than average 
...
$$
y_{ij} \sim Bernoulli(p_{ij}| \beta_1, \beta_2, \beta_{2Q}, \beta_3, \beta_4)
$$
...
$$\beta_2Q: \text{quadratic effect of background db}$$ 
...
cat("
    ...
    for(i in 1:n_unit){
      ...
      for(j in 1:miss[i]){
       # Impute missing detection data
        ...
        logit(theta[i,j]) <- b0 + b1*cov1[i] + b2*cov2[i,j] + b2Q*(cov2[i,j]^2) + b3*cov3[i,j] + b4*cov4[i,j]
  ...
  ", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Detection_Submodel/Model_Structure_Files/JAGS_DetMod_Fin3.txt")


```

```{R Run Model, results = FALSE}
habdet_f3 <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Detection_Submodel/Model_Structure_Files/JAGS_DetMod_Fin3.txt",
                 parameters.to.save = c("a0", "b0", "b1", "b2", "b2Q","b3", "b4"),
                 n.iter = 10000, 
                 n.burnin = 4000, 
                 n.chains = 3,
                 n.thin = 2,
                 inits = init_func_fin3)
beep()
```

```{R Save and Read in Model, eval = FALSE, include = FALSE}
saveRDS(habdet_f3, "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Detection_Submodel/Models_Ran_Outputs/Actual_Data/JAGS_DetModF3.Rdata")
habdet_f3 <- readRDS("C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Detection_Submodel/Models_Ran_Outputs/Actual_Data/JAGS_DetModF3.Rdata")
```

```{R}
summary(habdet_f3)
tracedens_jags(habdet_f3, parmfrow = c(3,3))
```
...
chains_f3 <- jags_df(habdet_f3)
# Select the chains for our covariates of interest
chains_viol <- chains_f3 %>% select(b1,b2,b2Q,b3,b4)
# Rename them to be more interpretable
colnames(chains_viol) <- c("veg density","background noise","quadratic background noise", "effort","date")
# Pivot this longer so that we can visualize it
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("veg density","background noise","quadratic background noise","effort","date"),names_to = "parameter", values_to = "values")
...
ggplot(data = chains_viol_long, aes(x = values, y = parameter, fill = parameter)) + 
  ...
  stat_slabinterval() +
  # Establish colors
  scale_fill_manual(values = c("veg density"=palette_5[1], "background noise" = palette_5[2],"quadratic background noise" = palette_8[1], "effort"=palette_5[3], "date" = palette_5[4], "quadratic date" = palette_5[5])) +
...
#### Build model with quadratic effect on either veg density and date

Should I build this separately from the rest of the model/do a univariate model?

#### WAIC: compare with other models

```{R Extract WAIC}
# Extract samples from the posterior
f3_s <- jags.samples(habdet_f3$model,
                           c("WAIC","deviance"),
                           type = "mean",
                           n.iter = 5000,
                           n.burnin = 1000,
...
f3_s$p_waic <- f3_s$WAIC
# Calculate a new waic by adding the waic plus the deviance
f3_s$waic_calc <- f3_s$deviance + f3_s$p_waic
...
waic_f3 <- round(c(waic = tmp[["waic_calc"]], p_waic = tmp[["p_waic"]]),1)
# Calculate the standard error for WAIC
f3_waic_se <- sd(f3_s$waic_calc)/(sqrt(length(f3_s$waic_calc)))
