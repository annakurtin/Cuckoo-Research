...
This is the ongoing iteration of the core home range (130 m radius of points) scale submodel within the occupancy/state submodel, including only height metrics, no standard deviation metrics. 

#### Model structure:

State model with four covariates: percent canopy (cova1) and subcanopy (cova2), height of canopy (cova3) and subcanopy (cova4)
...
cat("
  ...
  ", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Model_Structure_Files/JAGS_HabOccModC4a.txt")

```

```{R Run Model, results = FALSE}
fit_C4a <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Model_Structure_Files/JAGS_HabOccModC4a.txt",
...
```{R Evaluate Model}
print(fit_C4a)
tracedens_jags(fit_C4a, parmfrow = c(3,3))
```

```{R again plotting, echo = FALSE}
chains_c4a <- jags_df(fit_C4a)

chains_alpha <- chains_c4a %>% select(a1,a2,a3,a4)
colnames(chains_alpha) <- c("% canopy cover","% subcanopy cover", "canopy height", "subcanopy height")
chains_alpha_l <- chains_alpha %>% pivot_longer(cols = c("% canopy cover","% subcanopy cover", "canopy height", "subcanopy height"),names_to = "Parameter", values_to = "Values")
...
ggplot() +
  ...
  scale_fill_manual(values = c("% canopy cover"=palette_8[5], "% subcanopy cover" = palette_8[6], "canopy height" = palette_8[7], "subcanopy height" = palette_8[8])) + 
  geom_hline(yintercept = 0, linetype = "solid", color = "black") +
  theme_minimal()


chains_beta <- chains_c4a %>% select(b1, b1Q, b3, b4)
...
c4a_s <- jags.samples(fit_C4a$model,
                           c("WAIC","deviance"),
                           type = "mean",
                           n.iter = 5000,
                           n.burnin = 1000,
...
c4a_s$p_waic <- c4a_s$WAIC
# Calculate a new waic by adding the waic plus the deviance
c4a_s$waic_calc <- c4a_s$deviance + c4a_s$p_waic
# Create a new vector of the sum across each array (sum WAIC, sum deviance, sum p_waic, and sum waic_calc)
tmp <- sapply(c4a_s, sum)
# Pull out the sum of the WAIC we calculated and the p_waic we duplicated
waic_c4a <- round(c(waic = tmp[["waic_calc"]], p_waic = tmp[["p_waic"]]),1)
# Calculate the standard error for WAIC
c4a_waic_se <- sd(c4a_s$waic_calc)/(sqrt(length(c4a_s$waic_calc)))
```

```{R Colinearities with These Covariates}
cont_dat <- full_dat %>% select(tree_spp_rich, pct_can_landsc, pct_subcan_landsc, pct_can_core, pct_subcan_core, ht_can_core, ht_subcan_core)
corrgram(cont_dat, order=TRUE, lower.panel=panel.cor,
         upper.panel=panel.pts, text.panel=panel.txt,
         main="Correlations in the Continuous Data with Height")
