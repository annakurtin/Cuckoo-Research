...
I'll be using the same structure in this model, adding in the newly cleaned data and the new detection model that includes veg density as a covariate. 
...
$$ \alpha_1: \text{% subcanopy core area}$$

$$ \alpha_2: \text{canopy height core area}$$

$$ \alpha_3: \text{subcanopy height core area}$$
...
```{R}
subcancov <- full_dat %>% select(bbcu, pct_subcan_core)

subcancov_p1 <- subcancov %>% ggplot() + geom_histogram(aes(x = pct_subcan_core),fill = palette_8[2]) + theme_minimal()
subcancov_p2 <- ggplot(subcancov) + 
  geom_jitter(aes(x = pct_subcan_core, y = bbcu), color = palette_8[2], width = .05, height = .05) +
  geom_smooth(aes(x = pct_subcan_core, y = bbcu), color = palette_8[2], se = FALSE, method = "glm", method.args = list(family = "binomial")) +
  scale_y_continuous(breaks = seq(0, 1, by = 1)) +
  labs(x = "% Subcanopy Cover", y = "Detection") + theme_minimal()
```
...
htsubcan_p2 <- ggplot(ht_subcan) + 
  ...
  labs(x = "Subcanopy Height", y = "Detection") + 
  scale_y_continuous(breaks = seq(0, 1, by = 1)) +
  theme_minimal()
```
...
sdsubcan_p2 <- ggplot(sd_subcan) + 
  geom_jitter(aes(x = sd_subcan_core, y = bbcu), color = palette_8[5], width = .05, height = .05) +
  geom_smooth(aes(x = sd_subcan_core, y = bbcu), color = palette_8[5], se = FALSE, method = "glm", method.args = list(family = "binomial")) +
  labs(x = "Subcanopy St Dev", y = "Detection") + 
  scale_y_continuous(breaks = seq(0, 1, by = 1)) +
  theme_minimal()
```
...
cat("
  ...
  ", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Model_Structure_Files/JAGS_HabOccMod_CFin1.txt")

```


```{R Run Model, eval = FALSE, results = FALSE}
fit_CF1 <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Model_Structure_Files/JAGS_HabOccMod_CFin1.txt",
                 ...
                 inits = init_func)
beep()
saveRDS(fit_CF1,"C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Models_Ran_Outputs/StateSubModel_CFinal1.Rdata")
```

```{R read in model}
fit_CF1 <- readRDS("C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Models_Ran_Outputs/StateSubModel_CFinal1.Rdata")
```
...
```{R Evaluate Model}
summary(fit_CF1)
tracedens_jags(fit_CF1, parmfrow = c(2,3))
```

#### Examine model fit:

```{R Make Inference on P Value}
plot(fit_CF1$sims.list$SSEobs,fit_CF1$sims.list$SSEsim,xlab="SSEobs",ylab="SSEsim")
abline(0,1,lwd = 2, col = "red")
print(paste0("P value is: ",round(fit_CF1$mean$p_val, 2)))
```
...
chains_cf1 <- jags_df(fit_CF1)
# Select the chains for our covariates of interest
chains_viol <- chains_cf1 %>% select(a1,a2, a3, a4)
# Rename them to be more interpretable
colnames(chains_viol) <- c("% Subcanopy Cover","Canopy Height", "Subcanopy Height", "Veg SD Resid")
# Pivot this longer so that we can visualize it
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("% Subcanopy Cover","Canopy Height", "Subcanopy Height", "Veg SD Resid"),names_to = "parameter", values_to = "values")

f_stat <- data.frame(
  parameter = c("% Subcanopy Cover",
                "Canopy Height", 
                "Subcanopy Height", 
                "Veg SD Resid"),
  median_value = c(paste0("F: ",round(fit_CF1$f$a1,2)),
                   paste0("F: ",round(fit_CF1$f$a2,2)),
                   paste0("F: ",round(fit_CF1$f$a3,2)), 
                   paste0("F: ",round(fit_CF1$f$a4,2)))
)
...
ggplot(data = chains_viol_long, aes(x = values, y = parameter, fill = parameter)) + 
  ...
  stat_slabinterval() +
  # Establish colors
  scale_fill_manual(values = c("% Subcanopy Cover"=palette_8[2], "Canopy Height" = palette_8[3], "Subcanopy Height" = palette_8[4], "Veg SD Resid" = palette_8[7])) +
...
psi <- round(plogis(fit_CF1$q50$a0), 3)
print(paste0("psi:", psi))

a1 <- round(plogis(fit_CF1$q50$a0 + fit_CF1$q50$a1), 3)
print(paste0("% Subcanopy core:", a1))

a2 <- round(plogis(fit_CF1$q50$a0 + fit_CF1$q50$a2), 3)
print(paste0("Canopy Height:", a2))

a3 <- round(plogis(fit_CF1$q50$a0 + fit_CF1$q50$a3), 3)
print(paste0("Subcanopy Height:", a3))

a4 <- round(plogis(fit_CF1$q50$a0 + fit_CF1$q50$a4), 3)
...
mat <- cbind(fit_CF1$sims.list$a0,fit_CF1$sims.list$a1,fit_CF1$sims.list$a2,fit_CF1$sims.list$a3,fit_CF1$sims.list$a4)
...
* Residuals of all vegetation standard deviation regressed against canopy height: there is a 98% chance that the true effect of vegetation sd residuals on probability of use by BBCU is positive. The median estimate for vegetation sd residuals effect transformed into probability space is 0.306, which means that at the average values of every other covariate, for every 1 sd increase in veg sd residuals above the mean (.9 increase above mean value of 0 *not sure if it's helpful to include the values here???*), the probability of occupancy by BBCU increases by 30.6%.

**Weakly significant covariates (F > .70):**

* Subcanopy height: There is an 88% chance that the true effect of subcanopy height on probability of use by BBCU is positive. The median estiamte for the effect of subcanopy height transformed into probability space is 0.253, meaning that at the average values of the other covariates, for every 1 sd increase in subcanopy height above the mean (.4 increase above mean of 1.3 m), the probability of use by BBCU increases by 25.3%. However, this parameter is colinear with percent canopy core and canopy height, which means that the effect in subcanopy height doesn't vary without changing the effect of one of the other covariates. 

* Subcanopy cover: There is a 95% chance that the true effect of subcanopy cover on probability of use by BBCU is negative. The median estimate for the effect of subcanopy percent cover transformed into probability space is 0.108, meaning that at the mean values of the other covariates, for every 1 sd increase in subcanopyo height above the mean (15 percentage points increase above the average of 22), the probability of use by BBCU decreases by 10.8%.
