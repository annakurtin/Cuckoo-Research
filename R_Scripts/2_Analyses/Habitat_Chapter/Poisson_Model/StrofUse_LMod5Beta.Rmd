...
# create a new vector for these values so I can compare
days_orig <- days
n_unit <- nrow(full_dat)
# Transform days so that zeros are slightly above zero
days <- (days * (n_unit - 1) + 0.5) / n_unit
days[days>.05]
days_orig[days_orig>.5]
...
jags_data <- list(
  ...
  # % subcanopy landscape
  cov3 = full_dat$pct_openshrub_landsc,
  # residual veg sd
  cov4 = full_dat$veg_sd_resid,
  # % subcanopy core
  cov5 = full_dat$pct_openshrub_core,
  # year
...
cat("
  model {
    ...
    for (i in 1:n_unit) {
      ...
      # Beta regression
      mu_linear[i] <- a0 + a1*cov1[i] + a2*cov2[i] + a3*cov3[i] + a4*cov4[i] + a5*cov5[i] + a6*cov6[i]
...
", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Poisson_Model/Model_Structure_Files/JAGS_HabLinearModFinal4.txt")
...
fit_beta <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Poisson_Model/Model_Structure_Files/JAGS_HabLinearModFinal4.txt",
                 parameters.to.save = c("a0", "a1", "a2", "a3", "a4", "a5", "a6", "SSEobs", "SSEsim", "p_val"),
                 n.iter = 10000, 
                 n.burnin = 4000, 
                 n.chains = 3,
                 n.thin = 2,
...
saveRDS(fit_beta,"C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Poisson_Model/Models_Ran_Outputs/LModFinal4.Rdata")
```
```{R Read in Model}
fit_beta <- readRDS("C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Poisson_Model/Models_Ran_Outputs/LModFinal4.Rdata")
```

```{R Evaluate Model}
summary(fit_beta)
tracedens_jags(fit_beta, parmfrow = c(2,2)) # a0 looks weird here
# But if we plot it on the probability scale the value that it's jumping around is much more normal
plot(plogis(fit_beta$sims.list$a0))
# this model just looks like it needed more time to run
```
