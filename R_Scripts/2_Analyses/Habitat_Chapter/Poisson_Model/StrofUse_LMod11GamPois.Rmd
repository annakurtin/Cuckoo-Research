...
$$ c_i \sim Poisson(\gamma_i | \beta_1, \beta_2, \beta_3, \beta_4) $$
...
* Residuals of all vegetation standard deviation regressed against canopy height: we have no evidence that the true effect of vegetation sd residuals on the strength of use by BBCU is negative. The median estimate for vegetation sd residuals effect transformed into probability space is 0.604, which means that at the average values of every other covariate, for every 1 sd increase in veg sd residuals above the mean (.8 increase above mean value of 0 *not sure if it's helpful to include the values here???*), the count of BBCU calls during the breeding season increases by 60.4%.

* Percent shrub/early successional cover at landscape scale: we have no evidence that the true effect of percent shrub/early successional at landscpae sclae is positive. The median estimate for the effect of this covariate transformated into probability space is 0.111, which means that for every 1 sd increase in percent shrub/early successional cover across the landscape (increase of 7.64 percentage points above the mean value of 14.4% cover), the count of BBCU calls during the breeding season decreases by 11.1%. 

* Percent canopy cover at landscape scale: there is a 99% chance that the true effect of percent canopy cover at landscape scale is positive. The median estimate for the effect of this covariate transformed into probability space is 0.438, which means that for every 1 sd increase in percent canopy cover at the landscape scale (increase of 6.8 percentage points above the mean value of 8.2% cover), the count of BBCU calls during the breeding season increases by 43.8%. 

**Weakly significant covariates (F > .70):**

* Percent shrub/early successional cover at core scale: There is a 93% chance that the true effect of percent shrub/early successional cover at the core scale is negative. The median estimate for the effect of this covariate transformed into probability space is 0.165, which means that for every 1 sd increase in percent canopy cover at the core scale (increase of 11 percentage points above the mean value of 18.5% cover), the count of BBCU calls during the breeding season decreases by 16.5%.
