...
# Still not looking good, even after taking the year cvariate out. What to do next? Try to figure out if we can use all of the data in the model like Thomas was talking about
```


$$ \gamma_{1}: \text{percetn canopy} $$

$$ \gamma_{2}: \text{veg sd resid} $$
...
chains_beta <- jags_df(fit_gpois)
# Select the chains for our covariates of interest
chains_viol <- fit_gpois %>% select(a1, a2, a3)
# Rename them to be more interpretable
colnames(chains_viol) <- c("% Canopy Cover Landscape", "Veg SD Resid Core", "year")
# Pivot this longer so that we can visualize it
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("% Canopy Cover Landscape", "Veg SD Resid Core", "year"),names_to = "parameter", values_to = "values")

f_stat <- data.frame(
  parameter = c("% Canopy Cover Landscape", "Veg SD Resid Core", "year"),
  median_value = c(paste0("F: ",round(fit_gpois$f$a1,2)), 
                   paste0("F: ",round(fit_gpois$f$a2,2)),
                   paste0("F: ",round(fit_gpois$f$a3,2)))
  )
