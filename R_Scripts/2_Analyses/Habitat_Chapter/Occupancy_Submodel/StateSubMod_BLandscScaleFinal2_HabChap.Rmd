...
$$ \alpha_2: \text{Landscape percent cover of open shrubby area}$$
...
oshrubcov_p2 <- ggplot(oshrubcov) + 
  ...
  labs(x = "% Open Shrubby Cover", y = "Detection") + 
  scale_y_continuous(breaks = seq(0, 1, by = 1)) +
  theme_minimal()
```
...
cat("
  ...
  ", file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Model_Structure_Files/JAGS_HabOccMod_BFin2.txt")
```

```{R Run Model, eval = FALSE}
fit_BF2 <- jags(data = jags_data, 
                 model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Model_Structure_Files/JAGS_HabOccMod_BFin2.txt",
                 ...
                 inits = init_func)
beep()
saveRDS(fit_BF2,"C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/R_Scripts/2_Analyses/Habitat_Chapter/Occupancy_Submodel/Models_Ran_Outputs/StateSubModel_BFinal2.Rdata")
```
...
colnames(chains_viol) <- c("% Canopy Cover","% Open Shrub Cover")
# Pivot this longer so that we can visualize it
chains_viol_long <- chains_viol %>% pivot_longer(cols = c("% Canopy Cover","% Open Shrub Cover"),names_to = "parameter", values_to = "values")
...
print(paste0("% Open Shrub landscape:", a2))
```
...
* Percent subcanopy cover landscape: There is an 100% chance that the true effect of subcanopy cover at a landscape (1km) scale on BBCU probability of use during the breeding season is negative. The median estimate for the effect of subcanopy cover on use transformed into probability space is is 0.059, which means that for every 1 sd increase in percent subcanopy cover above the mean (increase of 8 percentage points over the mean of 16 percentage points), the probability of use by a BBCU decreases by 5.9%.
