...
a1 <- round(plogis(fit_CF1$q50$a1), 3)
print(paste0("% Subcanopy core:", a1))

a2 <- round(plogis(fit_CF1$q50$a2), 3)
print(paste0("Canopy Height:", a2))

a3 <- round(plogis(fit_CF1$q50$a3), 3)
print(paste0("Subcanopy Height:", a3))

a4 <- round(plogis(fit_CF1$q50$a4), 3)
...
* Residuals of all vegetation standard deviation regressed against canopy height: there is a 98% chance that the true effect of vegetation sd residuals on probability of use by BBCU is positive. The median estimate for vegetation sd residuals effect transformed into probability space is 0.673, which means that for every 1 sd increase in veg sd residuals above the mean (.9 increase above mean value of 0 *not sure if it's helpful to include the values here???*).

**Weakly significant covariates (F > .70):**

* Subcanopy height: There is an 88% chance that the true effect of subcanopy height on probability of use by BBCU is positive. The median estiamte for the effect of subcanopy height transformed into probability space is 0.621, meaning that for every 1 sd increase in subcanopy height above the mean (.4 increase above mean of 1.3 m), the probability of use by BBCU increases by 62.1%. However, this parameter is colinear with percent canopy core and canopy height, which means that the effect in subcanopy height doesn't vary without changing the effect of one of the other covariates. 

* Subcanopy cover: There is a 95% chance that the true effect of canopy cover on probability of use by BBCU is negative. The median estimate for the effect of subcanopy percent cover transformed into probability space is 0.37, meaning that for every 1 sd increase in subcanopyo height above the mean (15 percentage points increase above the average of 22), the probability of use by BBCU decreases by 37%.
