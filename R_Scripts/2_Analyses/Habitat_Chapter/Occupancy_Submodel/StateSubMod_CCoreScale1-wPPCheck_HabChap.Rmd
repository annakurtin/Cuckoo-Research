...
cat("
  model{
    ...
    for(i in 1:n_unit){
      ...
      for(j in 1:miss[i]){
        det_data[i,j] ~  dbern(mu_p[i,j])  # Create detection data drawn from a bernoulli distribution incorporating the true presence (Z) with probability of detection theta
        mu_p[i,j] <- Z[i]*theta[i,j]
        theta[i,j] <- logit(p[i,j])
        p[i,j] <- b0
        ...
        Presi[i,j] <- (det_data[i,j]-theta[i,j])^2 # Calculate squared residual of observed data
    ...
    ## Calculate sum of squared residual errors for observed data
    SSEobs <- sum(Presi[,])
...
fit_C1 <- jags(data = jags_data, 
                 ...
                 parameters.to.save = c("a0", "a1", "a2", "a3", "a4", "a5", "a6", "b0", "p_val","SSEobs", "SSEsim"),
                 n.iter = 10000, 
                 n.burnin = 4000, 
                 n.chains = 3,
                 n.thin = 2,
