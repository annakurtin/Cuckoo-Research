...
drich_p2 <- ggplot(drich) + 
  ...
  labs(x = "Deciduous Tree Spp Richness", y = "Detection") + theme_minimal()
```
...
crich_p2 <- ggplot(crich) + 
  ...
  labs(x = "Conifer Tree Spp Richness", y = "Detection") + theme_minimal()
```
...
fshrub_p2 <- ggplot(fshrub) + 
  ...
  labs(x = "Floodplain Shrub Community Dominant", y = "Detection") + theme_minimal()
```
    ...
    for(i in 1:n_unit){
      ...
      for(j in 1:miss[i]){
       # Impute missing detection data
        ...
        logit(theta[i,j]) <- b0 + b1*covb1[i] + b2*covb2[i,j] + b3*covb3[i,j] + b4*covb4[i,j] + b4Q*(covb4[i,j]^2)
...
fit_AF1 <- jags(data = jags_data, 
                 ...
                 parameters.to.save = c("a0", "a1", "a2", "a3","a4", "b0", "b1", "b3", "b4", "b4Q"),
                 n.iter = 10000, 
                 n.burnin = 4000, 
                 n.chains = 3,
                 n.thin = 2,
