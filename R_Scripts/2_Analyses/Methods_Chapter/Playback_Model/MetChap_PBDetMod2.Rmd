...
$$
p_{ij} = \beta_0 + \beta_{1}*date + \beta_{2}^2*date + \beta_3*wind + \beta_4*temp
$$
...
for (i in 1:nrow) { 
  ...
  for (j in 1:max_surveys){ 
    ...
    pstar[i,j] <- 1 - (1 - plogis(fit_2$sims.list$b0[i] + b1*0))^j #Calculate estimated maximum detection probability for each survey using mean probability calculated in the MCMC 
    #### ADD IN REST OF OCOVARITES
  } 
}
