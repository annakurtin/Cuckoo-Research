...
sink("C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/m1.jags")
cat("
      model {
...
for (ii in 1:nSims){  # use 'ii' here so it doesn't get mixed up with other loops, could use 'koala'
  ...
  m1 <- jags(data = jags.data, 
             ...
             model.file = "C:/Users/annak/OneDrive/Documents/UM/Research/Coding_Workspace/Cuckoo-Research/m1.jags",
             n.chains = nc,
             n.thin = nt,
             n.iter = ni,
             n.burnin = nb,
...
title(main = "Residuals for alpha 1")
...
#est.mu[1,1,1] # first row, first column of the first matrix (median)
#est.mu[1,1,2] # first row, first column of the second matrix (lower CI)

# colnames(diffs) <- c("psi","p","a1","a2","b1","b2")
# # Populate this data frame by calculating the values from each 
# for(sim in 1:nSims){
#   # Fill in the median for psi and p
#   diffs[sim,c("psi","p")] <- c(est.mu[sim,1,1],est.mu[sim,2,1])
#   # Fill in the median for alpha covariate 1 and 2
#   diffs[sim,c("a1","a2")] <- c(est.alpha[sim,1,1], est.alpha[sim,2,1])
#   # Fill in the median for the beta covariate 1 and 2
#   diffs[sim,c("b1","b2")] <- c(est.beta[sim,1,1], est.beta[sim,2,1])
# }
# plot(height = diffs, beside = TRUE)